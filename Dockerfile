...
RUN ["git", "clone", "-b", "v2.1.0/FastAPI", "https://github.com/hanul-pipeline/hanul-site-pipeline", "/hanul"]
...
RUN ["crontab", "/hanul/sh/cron"]

# ETC
# RUN NODE EXPORTER & APP
WORKDIR /hanul
CMD ["bash", "-c", "service cron start & uvicorn main:app --host 0.0.0.0 --port 9000 & node_exporter"]
